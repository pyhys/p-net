...
int pnal_execute_script (const char * argv[])
{
   ...
   if (argv == NULL)
   {
      LOG_ERROR (PF_PNAL_LOG, "PNAL(%d): No argument vector given for running script.\n", __LINE__);
      return -1;
   }
   ...
   {
      LOG_ERROR (PF_PNAL_LOG, "PNAL(%d): Could not build PATH to run script.\n", __LINE__);
      return -1;
   }
   ...
   else if (childpid == 0)
   {
      ...
      {
         printf ("PNAL(%d): Failed to set PATH in child process to run script.\n", __LINE__);
         exit (EXIT_FAILURE);
      }

      execvpe (argv[0], (char * const *)argv, scriptenviron);

      printf ("PNAL(%d): Failed to execute in child process. Is the script file missing or lacks execution permission? %s Search path %s\n", __LINE__, argv[0], child_searchpath);
      exit (EXIT_FAILURE);
   }
   else
   {
