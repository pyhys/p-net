   ...
   if (p_iocr->cpm.ci_running == true) /* Timer running */
   {
      ...
      case PF_CPM_STATE_RUN:
         if (p_iocr->cpm.dht >= p_iocr->cpm.data_hold_factor)
         {
			...
			//if the AR is still in use an alarm should be sent
			if (p_iocr->p_ar && p_iocr->p_ar->in_use){
   ...
   void * p_arg)
{
   ...
   case PF_CPM_STATE_RUN:
      ...
      else
      {
         ...
         LOG_DEBUG (
            PF_CPM_LOG,
            "CPM(%d): data_valid != false && dht_reload == 0\n",
            __LINE__);
      }
...
int pf_cpm_activate_req (pnet_t * net, pf_ar_t * p_ar, uint32_t crep)
{
   ...
   case PF_CPM_STATE_W_START:
      ...
      /* ToDo: Shall be aligned with local send clock or PTCP (Does it matter
       * for RTClass1/2?) */
         ...
         else
         {
            ...
            else
            {
               ...
               *p_new_flag = false;
               LOG_DEBUG (
                  PF_CPM_LOG,
                  "CPM(%d): No data received in get data\n",
                  __LINE__);
            }
         }
         break;
      ...
      case PF_CPM_STATE_RUN:
         ...
         else
         {
            ...
            else
            {
               LOG_DEBUG (
                  PF_CPM_LOG,
                  "CPM(%d): No data received in get iocs\n",
                  __LINE__);
            }
         }
         break;
