...
void pf_lldp_get_chassis_id (pnet_t * net, pf_lldp_chassis_id_t * p_chassis_id)
{
   ...
   const pf_lldp_name_of_device_mode_t mode =
      pf_port_get_lldp_name_device_mode (net);
   ...
   if (mode == PF_LLDP_NAME_OF_DEVICE_MODE_LEGACY)
   {
      if (
         p_chassis_id->len == 0 ||
...
int pf_lldp_get_system_description (
   const pnet_t * net,
   char * system_description,
   uint16_t system_description_max_len)
{
   return snprintf (
      system_description,
...
int pf_lldp_mac_to_str (
   const uint8_t * mac,
   char * mac_str,
   uint16_t mac_str_max_len)
{
   return snprintf (
      mac_str,
      mac_str_max_len,
