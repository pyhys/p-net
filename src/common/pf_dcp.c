 ...
 * @param mac              In: Destination MAC address
 * @return true if destination address is ok and shall packet shall
 *         be accepted. false if not.
 */
static bool pf_dcp_check_destination_address (
   const pnet_t * net,
   const pnet_ethaddr_t * mac)
{
   const pnet_ethaddr_t * local_mac = pf_cmina_get_device_macaddr (net);
   return (
      ((mac->addr[0] & 1) == 0) && /* Not multi-cast */
      (memcmp (mac, local_mac, sizeof (*mac)) == 0));
}
   ...
   void * p_arg)
{
   ...
   if (p_buf != NULL)
   {
      ...
      if (
         ...
         (pf_dcp_check_destination_address (net, &p_src_ethhdr->dest) == true))
      {
