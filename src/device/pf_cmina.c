...
void pf_cmina_port_statistics_show (pnet_t * net)
{
   ...
   const pnet_port_cfg_t * p_port_config = NULL;

   if (pnal_get_port_statistics (net->fspm_cfg.if_cfg.main_port.if_name, &stats) == 0)
   {
      printf (
         "Main interface %s    In: %" PRIu32 " bytes %" PRIu32 " errors %" PRIu32
         " discards  Out: %" PRIu32 " bytes %" PRIu32 " errors %" PRIu32
         " discard"
         "s\n",
         net->fspm_cfg.if_cfg.main_port.if_name,
         stats.if_in_octets,
   ...
   {
      printf ("Did not find main interface %s\n", net->fspm_cfg.if_cfg.main_port.if_name);
   }
   ...
   while (port != 0)
   {
      ...
      {
         printf (
            "Port %s    In: %" PRIu32 " bytes %" PRIu32 " errors %" PRIu32
            " discards  Out: %" PRIu32 " bytes %" PRIu32 " errors %" PRIu32
            " discard"
            "s\n",
